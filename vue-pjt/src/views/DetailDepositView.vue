    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">{{ defaultDeposit[0].kor_co_nm }}의 적금 목록입니다.</h1>
    ...
    <div class="toptop">
        ...
        <thead>
            <tr>
                ...
                <td>{{ defaultDeposit[0].fin_prdt_nm }}</td>
            </tr>
            <tr>
                <td class="title">회사 이름</td>
                <td>{{ defaultDeposit[0].kor_co_nm }}</td>
            </tr>
            <tr>
                <td class="title">유의 사항</td>
                <td>{{ defaultDeposit[0].etc_note }}</td>
            </tr>
            <tr>
                <td class="title">가입 방법</td>
                <td>{{ defaultDeposit[0].join_way }}</td>
            </tr>
            <tr>
                <td class="title">만기 후 이자율</td>
                <td>{{ defaultDeposit[0].mtrt_int }}</td>
            </tr>
            <tr>
                <td class="title">최고 한도</td>
                <td>{{ defaultDeposit[0].max_limit }}</td>
            </tr>
            <tr>
                <td class="title">가입 제한</td>
                <td>{{ defaultDeposit[0].join_deny }}</td>
            </tr>
            <tr>
                <td class="title">가입 대상</td>
                <td>{{ defaultDeposit[0].join_member }}</td>
            </tr>
            <tr>
                <td class="title">우대 조건</td>
                <td>{{ defaultDeposit[0].spcl_cnd }}</td>
            </tr>
        </thead>
      </table>
...
const savingconfirm = function() {
    changefunction()
    store.DepositData(id, 1)
}

const savingcancel = function() {
    changefunction()
    store.DepositData(id, 2)
}

const defaultDeposit = computed(() => {
    return store.deposits.filter(depo => depo.fin_prdt_cd.includes(id) || [1])
})

const filterSaving = computed(() => {
    return store.fullDepositOptions.filter(sav => sav.fin_prdt_cd.includes(id))
})
